...
def generate_synthetic_data(model_name, seed, prompt, n=500, max_attempts=3) -> (dict, dict):
    generation_log = {
        "config": {
...
def generate_synthetic_sst2(model_name, seed, sentiment, label_id, n=500, max_attempts=3):
...
Example format: ["review 1", "review 2", ...]"""
    ...
    sentences, generation_log = generate_synthetic_data(model_name, seed, prompt, n, max_attempts)

    return [{"sentence": s, "label": label_id} for s in sentences], generation_log

def generate_synthetic_snli(model_name, seed, relation, label_id, n=300, max_attempts=3):
...
Example: [{{"premise": "A man is walking.", "hypothesis": "Someone is moving."}}]"""
    ...
    pairs, generation_log = generate_synthetic_data(model_name, seed, prompt, n, max_attempts)
